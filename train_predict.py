...
Updated to pass benchmark parameter for relative strength calculation.
"""
...
def main():
    ...
    equity_signal = process_module(
        ...
        EQUITY_ETFS, 
        EQUITY_REGIME, 
        EQUITY_BENCHMARK,  # NEW: Pass benchmark for RS
        option, 
        HORIZONS, 
        ETA, 
        N_PATHS, 
    ...
    if not equity_signal or 'error' in equity_signal:
        ...
        sys.exit(1)
    print(f"Equity processing complete. Signal: {equity_signal.get('selected_etfs', 'ERROR')}")
    ...
    fi_signal = process_module(
        ...
        FI_ETFS, 
        FI_REGIME, 
        FI_BENCHMARK,  # NEW: Pass benchmark for RS
        option, 
        HORIZONS, 
        ETA, 
        N_PATHS, 
    ...
    if not fi_signal or 'error' in fi_signal:
        ...
        sys.exit(1)
    print(f"Fixed Income processing complete. Signal: {fi_signal.get('selected_etfs', 'ERROR')}")
